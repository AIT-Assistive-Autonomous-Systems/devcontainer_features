...
ADD_GROUPS=$(cat /host_groups_test)
...
cat << EOF > /tmp/add_groups.sh
#!/bin/sh
true
EOF

check "verify empty post-create" diff /tmp/add_groups.sh /usr/local/share/AIT-Assistive-Autonomous-Systems/devcontainer_features/add_groups.sh
